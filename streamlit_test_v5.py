...
def _load_pricing_from_disk() -> Optional[Dict[str, Dict[str, float]]]:
    ...
    try:
        file_age_days = (time.time() - _OPENROUTER_PRICING_FILE.stat().st_mtime) / (60 * 60 * 24)

        if file_age_days > _PRICING_CACHE_TTL_DAYS:
            # Cache expired
            return None
...
def fetch_openrouter_pricing() -> Dict[str, Dict[str, float]]:
    """
    Fetches and caches all OpenRouter model pricing data for 30 days.

    This function:
    1. First checks for locally saved pricing data on disk
    2. If found and not expired (< 30 days old), returns cached data
    3. Otherwise, fetches fresh pricing from OpenRouter API
    4. Saves the fresh data to disk for future use
    """
    # Try to load from disk first
    cached_pricing = _load_pricing_from_disk()
    ...
    try:
        with httpx.Client(timeout=10) as client:
            # Fetch all models (no filtering) to get the most comprehensive price list
            ...
            for m in data.get('data', []):
                ...
                if 'prompt' in pricing and 'completion' in pricing:
                    ...
                    pricing_map[model_id] = {
                        "input_per_mtok_usd": pricing['prompt'] * 1_000_000,
                        "output_per_mtok_usd": pricing['completion'] * 1_000_000,
                    }
...
# --- NEW: Model ID Conversion Helpers ---
def _to_openrouter_model_id(model: str, provider: str = None) -> str:
    """
    Convert a model ID to OpenRouter format (with provider prefix).

    Args:
        model: Model ID (e.g., "gemini-2.5-flash" or "google/gemini-2.5-flash")
        provider: Provider hint ("google", "openai", etc.) - auto-detected if not provided

    Returns:
        OpenRouter-formatted model ID (e.g., "google/gemini-2.5-flash")
    """
...
def _to_native_model_id(model: str) -> str:
    """
    Convert a model ID to native API format (strip provider prefix).

    Args:
        model: Model ID (e.g., "google/gemini-2.5-flash" or "gemini-2.5-flash")

    Returns:
        Native API model ID (e.g., "gemini-2.5-flash")
    """
    if "/" in model:
...
# --- NEW: Fetch models from OpenRouter API with accurate pricing ---
...
def _fetch_models_from_openrouter(provider_filter: str = None) -> Dict[str, Dict[str, Any]]:
    """
    Fetch model information from OpenRouter API with accurate pricing.

    Args:
        provider_filter: Filter models by provider (e.g., "google", "openai")

    Returns:
        Dict mapping model_id to metadata (context, pricing, etc.)
    """
    if not OPENROUTER_API_KEY:
        return {}
    ...
    try:
        with httpx.Client(timeout=10) as client:
            ...
            for m in data.get('data', []):
                ...
                if provider_filter:
                    if provider_filter.lower() == "google" and not model_id.startswith("google/gemini-2.5"):
                        continue
                    elif provider_filter.lower() == "openai" and not model_id.startswith("openai/gpt-5"):
                        continue
                ...
                if 'prompt' in pricing and 'completion' in pricing:
                    ...
                    models[model_id] = {
                        'context': f"{context_length:,}" if context_length else 'N/A',
                        'input_cost': f"${input_per_mtok:.3f}" if input_per_mtok > 0 else 'Free',
                        'output_cost': f"${output_per_mtok:.3f}" if output_per_mtok > 0 else 'Free',
        ...
        st.warning(f"⚠️ Failed to fetch models from OpenRouter: {e}")
        return {}

# --- PATCH 31: Dynamic Gemini Model Discovery via Linkup API ---
@st.cache_data(ttl=60 * 60 * 24 * 30)  # Cache for 30 days
def fetch_gemini_models_from_linkup() -> Dict[str, Dict[str, Any]]:
    """
    Fetch latest Gemini models using Linkup API to search Google's documentation.
    ...
    try:
        ...
        if not linkup_api_key:
            st.warning("⚠️ LINKUP_API_KEY not set. Using default Gemini models.")
            return _get_default_gemini_models()

        # Search for latest Gemini models
        headers = {
        ...
        payload = {
            "q": "Google Gemini API models list 2025 available models gemini-2.0 gemini-1.5 pricing context window",
            "depth": "standard",
            "outputType": "sourcedAnswer",
            "includeImages": False,
            "includeDomains": ["ai.google.dev", "cloud.google.com", "developers.google.com"],
        ...
        models = _parse_gemini_models_from_linkup(data)

        if models:
            st.success(f"✅ Discovered {len(models)} Gemini models via Linkup API (cached for 30 days).")
            return models
        else:
...
def render_test_flow_diagram(test_number: int, test_name: str):
    ...
    flow = flows.get(test_number, "📊 Data → 🤖 Process → 📈 Evaluate")
...
def visualize_dataset_composition(df: pd.DataFrame, dataset_type: str = "classification"):
    ...
    if kind in ("tool_agent_sequence", "tool/agent_sequence", "tool_agent", "tool_sequence"):
        ...
        if seq_col:
            ...
            try:
                ...
                st.plotly_chart(fig_len, width='stretch', config=PLOTLY_CONFIG)
            except Exception:
                pass
    ...
    if kind in ("context_pruning", "pruning", "context"):
        ...
        if "expected_action" in df.columns:
            ...
            st.plotly_chart(fig_actions, width='stretch', config=PLOTLY_CONFIG)
        ...
        if "expected_kept_keys" in df.columns and df["expected_kept_keys"].notna().any():
            try:
                ...
                if keys:
                    ...
                    st.plotly_chart(fig_keys, width='stretch', config=PLOTLY_CONFIG)
            except Exception:
                pass
...
def render_organized_results(df: pd.DataFrame, test_type: str = "classification", model_cols: List[str] = None, model_names: List[str] = None):
                    ...
                    if result_col in df.columns:
                        with col:
                            ...
                            if valid_indices:
                                ...
                                st.dataframe(pd.DataFrame(report_dict).transpose().style.format("{:.2f}"), width='stretch')
    ...
    with result_tabs[2]:
        ...
        if test_type == "classification" and "classification" in df.columns and model_cols:
            ...
            st.dataframe(view_df[existing_cols], width='stretch')
...
with tabs[0]:
    ...
    if any([os.path.exists(p) for p in [CLASSIFICATION_DATASET_PATH, TOOL_SEQUENCE_DATASET_PATH, CONTEXT_PRUNING_DATASET_PATH]]):
        ...
        with analytics_tabs[0]:
            ...
            st.dataframe(style_stats(stats_df), width='stretch')
            ...
            with comp_col1:
                ...
                if valid_stats:
                    ...
                    st.plotly_chart(fig_items, width='stretch', config=PLOTLY_CONFIG)
                else:
            ...
            with comp_col2:
                ...
                if valid_stats:
                    ...
                    st.plotly_chart(fig_complexity, width='stretch', config=PLOTLY_CONFIG)
                else:
                    st.info("No datasets available for comparison")
        ...
        with analytics_tabs[1]:
            ...
            if not gen_history:
                ...
                st.plotly_chart(fig_cost_trend, width='stretch', config=PLOTLY_CONFIG)
                ...
                st.plotly_chart(fig_tokens, width='stretch', config=PLOTLY_CONFIG)
                ...
                if unique_models >= 2:
                    ...
                    with eff_col1:
                        ...
                        st.plotly_chart(fig_cost_eff, width='stretch', config=PLOTLY_CONFIG)
                    ...
                    with eff_col2:
                        ...
                        st.plotly_chart(fig_tokens_eff, width='stretch', config=PLOTLY_CONFIG)
                ...
                if os.path.exists(CLASSIFICATION_DATASET_PATH):
                    try:
                        ...
                        if not df_class.empty and 'classification' in df_class.columns:
                            ...
                            st.plotly_chart(fig_gauge_class, width='stretch', config=PLOTLY_CONFIG)
                            ...
                            if 'query' in df_class.columns:
                                ...
                                if all_words:
                                    ...
                                    st.plotly_chart(fig_keywords, width='stretch', config=PLOTLY_CONFIG)
                                else:
                ...
                if os.path.exists(TOOL_SEQUENCE_DATASET_PATH):
                    try:
                        ...
                        if not df_tool.empty and 'expected_sequence' in df_tool.columns:
                            ...
                            st.plotly_chart(fig_gauge_tool, width='stretch', config=PLOTLY_CONFIG)
                            ...
                            st.plotly_chart(fig_seq_dist, width='stretch', config=PLOTLY_CONFIG)
                ...
                if os.path.exists(CONTEXT_PRUNING_DATASET_PATH):
                    try:
                        ...
                        if not df_prune.empty and 'expected_action' in df_prune.columns:
                            ...
                            st.plotly_chart(fig_gauge_prune, width='stretch', config=PLOTLY_CONFIG)
        ...
        with analytics_tabs[3]:
            ...
            if dataset_token_data:
                ...
                if violin_data:
                    ...
                    st.plotly_chart(fig_violin, width='stretch', config=PLOTLY_CONFIG)
                ...
                if selected_model:
                    ...
                    st.plotly_chart(fig_costs, width='stretch', config=PLOTLY_CONFIG)
                    ...
                    st.plotly_chart(fig_io_costs, width='stretch', config=PLOTLY_CONFIG)
    ...
    # Generate button
    if st.button(f"🔄 Generate All Datasets ({selected_use_case})", width='stretch', type="primary"):
        try:
    ...
    if st.session_state.get("last_generated_meta_files"):
        with st.expander("🧾 View last generation metadata"):
            ...
            if rows:
                try:
                    st.dataframe(pd.DataFrame(rows), width='stretch', height=min(300, 60+28*len(rows)))
                except Exception:
                    st.write(rows)
            # Raw JSON entries
            for p in files:
    ...
    if st.session_state.get("generation_history"):
        with st.expander("🗂️ Generation history"):
            ...
            try:
                ...
                st.dataframe(dfh[["when","use_case","model","routing","total_items","tokens","cost_usd","calls"]].sort_values("when", ascending=False), width='stretch')
            except Exception:
                st.write(hist)
    ...
    if st.button(f"🎲 Randomly Generate Prompt for {data_type}", width='stretch'):
    ...
    if selected_prompts:
        ...
        for i, prompt in enumerate(selected_prompts):
            with cols[i]:
                if st.button(prompt[:30] + "...", key=f"pill_{data_type}_{i}", help=prompt, width='stretch'):
    ...
    if st.button("Generate & Save Dataset", width='stretch'):
...
with tabs[1]:
    ...
    st.dataframe(_style_selected_rows(_df_preview, ROW_LIMIT_N), width='stretch', height=250)
    ...
    if st.button("▶️ Run Test 1", type="primary", width='stretch'):
...
with tabs[2]:
    ...
    if st.button("▶️ Run Test 2", type="primary", width='stretch'):
        # Persist per-test OpenAI model override for this run
...
with tabs[3]:
    ...
    if st.button("▶️ Run Test 3 (Judge)", type="primary", width='stretch'):
        # Persist per-test OpenAI model override for this run
        ...
        if not len(df):
            ...
            with st.spinner("Running Judge on classified rows..."):
                ...
                st.plotly_chart(fig_sankey, width='stretch', config=PLOTLY_CONFIG)
                ...
                with st.expander("👨‍⚖️ Judge Performance (Detailed)", expanded=True):
                    ...
                    st.dataframe(judge_display_df, width='stretch')
...
with tabs[4]:
    ...
    if not st.session_state.pruning_df.empty:
        ...
        st.dataframe(pruning_df, width='stretch')
    ...
    if st.button("▶️ Run Test 4 (Pruning)", type="primary", width='stretch') and pruning_df is not None:
        ...
        with viz_col2:
            ...
            if key_action_data:
                ...
                st.plotly_chart(fig_stacked, width='stretch', config=PLOTLY_CONFIG)
            else:
...
if show_main_df_previews:
    ...
    with col1:
        ...
        st.download_button("⬇️ Download Results as CSV", data=csv_bytes, file_name="classification_results.csv", mime="text/csv", width='stretch')
    with col2:
